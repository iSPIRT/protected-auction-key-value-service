...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
ABSL_FLAG(std::string, ann_index_file_path, "", "Path to file with ann index.");
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
absl::Status TestUdf(const std::string& kv_delta_file_path,
                     ...
                     const std::string& ann_index_file_path) {
#else
absl::Status TestUdf(const std::string& kv_delta_file_path,
                     const std::string& udf_delta_file_path,
                     const std::string& input_arguments) {
  ...
  auto kv_load_status = absl::OkStatus();
  if (!kv_delta_file_path.empty()) {
    kv_load_status = LoadCacheFromFile(log_context, kv_delta_file_path, *cache);
    if (!kv_load_status.ok() && !kv_delta_file_path.empty()) {
      LOG(ERROR) << "Error loading cache from file: " << kv_load_status;
      return kv_load_status;
    } else {
      cache_loaded = true;
    }
  }
  ...
  microsoft::ANNIndex microsoft_ann_index;
  microsoft::SNAPSHOT_STATUS ann_load_status =
      microsoft::SNAPSHOT_STATUS::IN_PROGRESS;
  if (!ann_index_file_path.empty()) {
    LOG(INFO) << "Loading ann index from ann snapshot: " << ann_index_file_path;
    ann_load_status = microsoft_ann_index.TryAddANNSnapshot(
        "TestSnapshot", ann_index_file_path, log_context);
    if (ann_load_status != microsoft::SNAPSHOT_STATUS::OK) {
      LOG(ERROR) << "Error loading ann index from file: "
                 << ann_index_file_path;
    } else {
      ann_loaded = true;
    }
  }
  if (!cache_loaded && !ann_loaded) {
    LOG(ERROR) << "Error loading cache or ann index - both can not be null; "
               << "ANN load status: " << (size_t)ann_load_status
               << ", Cache load status: " << kv_load_status;
    return kv_load_status;
  }
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  auto get_nearest_neighbors_hook = GetNearestNeighborsHook::Create();
  get_nearest_neighbors_hook->FinishInit(
      microsoft::AnnLookup::CreateAnnLookup(microsoft_ann_index));
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
              .MicrosoftRegisterGetNearestNeighborsHook(
                  *get_nearest_neighbors_hook)
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  const std::string ann_index_file_path =
      absl::GetFlag(FLAGS_ann_index_file_path);
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)

#if defined(MICROSOFT_AD_SELECTION_BUILD)
  auto status = kv_server::TestUdf(kv_delta_file_path, udf_delta_file_path,
                                   input_arguments, ann_index_file_path);
#else
  auto status = kv_server::TestUdf(kv_delta_file_path, udf_delta_file_path,
                                   input_arguments);
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
