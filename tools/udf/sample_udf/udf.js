...
function getKeyGroupOutputs(udf_arguments) {
  let keyGroupOutputs = [];
  // MICROSOFT CODE START HERE
  let MICROSOFT_ANN_EMB_PREFIX = 'ANN_EMB_';
  // MICROSOFT CODE END HERE
  for (let argument of udf_arguments) {
    ...
    // MICROSOFT CODE START HERE
    /* code below is commented to see what was here before changes
    const getValuesResult = JSON.parse(getValues(data));
    */
    let microsoft_data_for_kv = data.filter((key) => !key.startsWith(MICROSOFT_ANN_EMB_PREFIX));
    const getValuesResult = JSON.parse(getValues(microsoft_data_for_kv));
    // MICROSOFT CODE END HERE
    ...
    // MICROSOFT CODE START HERE
    let keyGroupAnnOutput = {};
    keyGroupAnnOutput.tags = argument.tags;
    let data_for_ann = data
      .filter((key) => key.startsWith(MICROSOFT_ANN_EMB_PREFIX))
      .map((key) => key.slice(MICROSOFT_ANN_EMB_PREFIX.length));
    if (data_for_ann.length === 0) {
      continue;
    }
    const getNearestNeighborsResult = JSON.parse(getNearestNeighbors(data_for_ann));
    ...
    if (getNearestNeighborsResult.hasOwnProperty('kvPairs')) {
      ...
      keyGroupAnnOutput.keyValues = keyValuesOutput;
      keyGroupOutputs.push(keyGroupAnnOutput);
    }
    // MICROSOFT CODE END HERE
  }
