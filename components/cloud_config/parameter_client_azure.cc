...
ABSL_FLAG(absl::Duration, azure_poll_frequency, absl::Seconds(5),
...
ABSL_FLAG(int32_t, azure_realtime_updater_num_threads, 1,
...
ABSL_FLAG(bool, route_v1_to_v2, true,
...
class AzureParameterClient : public ParameterClient {
 public:
  AzureParameterClient(
    ...
    string_flag_values_.insert(
        {"kv-server-azure-launch-hook", absl::GetFlag(FLAGS_launch_hook)});
    string_flag_values_.insert({"kv-server-azure-realtime-directory",
    ...
    string_flag_values_.insert({"kv-server-azure-consented-debug-token",
    ...
    int32_t_flag_values_.insert(
        {"kv-server-azure-backup-poll-frequency-secs",
         absl::ToInt64Seconds(absl::GetFlag(FLAGS_azure_poll_frequency))});
    int32_t_flag_values_.insert(
        {"kv-server-azure-realtime-updater-num-threads",
         absl::GetFlag(FLAGS_azure_realtime_updater_num_threads)});
    int32_t_flag_values_.insert(
        {"kv-server-azure-data-loading-num-threads",
         absl::GetFlag(FLAGS_data_loading_num_threads)});
    int32_t_flag_values_.insert(
        {"kv-server-azure-num-shards", absl::GetFlag(FLAGS_num_shards)});
    int32_t_flag_values_.insert({"kv-server-azure-udf-num-workers",
                                 absl::GetFlag(FLAGS_udf_num_workers)});
    int32_t_flag_values_.insert({"kv-server-azure-logging-verbosity-level",
    ...
    int32_t_flag_values_.insert(
        {"kv-server-azure-udf-timeout-millis",
         absl::ToInt64Milliseconds(absl::GetFlag(FLAGS_udf_timeout))});
    int32_t_flag_values_.insert(
        {"kv-server-azure-udf-update-timeout-millis",
         absl::ToInt64Milliseconds(absl::GetFlag(FLAGS_udf_update_timeout))});
    int32_t_flag_values_.insert({"kv-server-azure-udf-min-log-level",
                                 absl::GetFlag(FLAGS_udf_min_log_level)});
    // Insert more int32 flag values here.
    bool_flag_values_.insert({"kv-server-azure-route-v1-to-v2",
                              absl::GetFlag(FLAGS_route_v1_to_v2)});
    bool_flag_values_.insert({"kv-server-azure-add-missing-keys-v1",
  ...
  absl::StatusOr<std::string> GetParameter(
    ...
    if (it != string_flag_values_.end()) {
      ...
      return absl::InvalidArgumentError(
          absl::StrCat("Unknown azure string parameter: ", parameter_name));
    }
  };
  ...
  absl::StatusOr<int32_t> GetInt32Parameter(
    ...
    if (it != int32_t_flag_values_.end()) {
      ...
      return absl::InvalidArgumentError(
          absl::StrCat("Unknown azure int32_t parameter: ", parameter_name));
    }
  }
  ...
  absl::StatusOr<bool> GetBoolParameter(
    ...
    if (it != bool_flag_values_.end()) {
      ...
      return absl::InvalidArgumentError(
          absl::StrCat("Unknown azure bool parameter: ", parameter_name));
    }
  }
...
std::unique_ptr<ParameterClient> ParameterClient::Create(
    ...
    privacy_sandbox::server_common::log::PSLogContext& log_context) {
  return std::make_unique<AzureParameterClient>(log_context);
}
