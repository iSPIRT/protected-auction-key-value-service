...
std::string MicrosoftGetFullPathForLocation(
    const BlobStorageClient::DataLocation& location) {
  // blob_storage_client_local -> FileBlobStorageClient.GetFullPath
  // (private method)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
    auto ann_status = MicrosoftLoadAnnSnapshotFiles(options);
    if (!ann_status.ok()) {
      // ann is not mandatory to use
    }
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
      // both functions are checking filename for regex
      // IsDeltaFilename checks for "^DELTA_\d{16}$"
      // MicrosoftIsAnnSnapshotFilename checks for "^ANNSNAPSHOT_\d{16}$"
      // DELTA_ is used for lookup deltas and ANNSNAPSHOT_ is used for ANN
      // snapshots as the data is fundamentally different so we'll accept both
      // as valid filename prefixes.
      if (!IsDeltaFilename(blob.key) &&
          !MicrosoftIsAnnSnapshotFilename(blob.key)) {
#else
      if (!IsDeltaFilename(blob.key)) {
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
      // TODO: make with RetryUntilOk task.ms/id56522398
      if (MicrosoftIsAnnSnapshotFilename(blob.key)) {
        auto annsnapshot_location =
            BlobStorageClient::DataLocation{.bucket = options_.data_bucket,
                                            .prefix = blob.prefix,
                                            .key = blob.key};
        auto status = options_.microsoft_ann_index.TryAddANNSnapshot(
            blob.key, MicrosoftGetFullPathForLocation(annsnapshot_location),
            options_.log_context);
        if (status != microsoft::SNAPSHOT_STATUS::OK) {
          PS_LOG(WARNING, options_.log_context)
              << "Failed to add ann snapshot file: "
              << MicrosoftGetFullPathForLocation(annsnapshot_location)
              << ", status is " << (size_t)status;
        }
      } else {
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
        RetryUntilOk(
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  // Loads ann snapshot files if there are any.
  // Loads it directly to the ann index.
  static absl::Status MicrosoftLoadAnnSnapshotFiles(const Options& options) {
    bool found = false;
    for (const auto& prefix : options.blob_prefix_allowlist.Prefixes()) {
      auto location = BlobStorageClient::DataLocation{
          .bucket = options.data_bucket, .prefix = prefix};
      PS_LOG(INFO, options.log_context)
          << "Initializing ann index with ann snapshot file(s) from: "
          << location;
      PS_ASSIGN_OR_RETURN(
          auto annsnapshot_groups,
          FindFileGroups(
              location,
              FileGroupFilter{.file_type = FileType::ANNSNAPSHOT,
                              .status = FileGroup::FileStatus::kComplete},
              options.blob_client));
      if (annsnapshot_groups.empty()) {
        PS_LOG(INFO, options.log_context)
            << "No ann snapshot files found in: " << location;
        continue;
      }
      // files are sorted by creation time, so we need to iterate in reverse
      for (auto it = annsnapshot_groups.rbegin();
           it != annsnapshot_groups.rend(); ++it) {
        // currently ANNIndex supports only one file in a group.
        // So we are taking random one (first one).
        std::string annsnapshot = *it->Filenames().begin();
        auto annsnapshot_location =
            BlobStorageClient::DataLocation{.bucket = options.data_bucket,
                                            .prefix = prefix,
                                            .key = annsnapshot};
        auto status = options.microsoft_ann_index.TryAddANNSnapshot(
            annsnapshot, MicrosoftGetFullPathForLocation(annsnapshot_location),
            options.log_context);
        if (status == microsoft::SNAPSHOT_STATUS::OK) {
          // at the moment when we found valid ann snapshot we can stop.
          found = true;
          break;
        } else {
          // microsoft::SNAPSHOT_STATUS::NOT_FRESH is impossible here, so it
          PS_LOG(ERROR, options.log_context)
              << "Some error while trying to add new snapshot at location: "
              << location << ", error was " << (size_t)(status);
          // should be an error in case if snapshot was not added we need to try
          // with previous one.
          continue;
        }
      }
    ...
    if (found) {
      // it is done INTENTIONALLY to return from this function OUTSIDE of loop
      // if we add return inside iteration absl::flat_hash_set this hash_set
      // will be corrupted it will cause double free and crash when try to
      // destroy BlobPrefixAllowlist inside DataOrchestrator::Options.
      return absl::OkStatus();
    }
    return absl::NotFoundError("No valid ann snapshots found");
  }
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
