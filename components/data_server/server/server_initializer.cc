...
void InitializeUdfHooksInternal(
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
    std::function<std::unique_ptr<Lookup>()> get_ann_lookup,
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  PS_VLOG(9, log_context) << "Finishing getNearestNeighbors init";
  get_nearest_neighbors_hook.FinishInit(get_ann_lookup());
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
  ...
  PS_VLOG(9, log_context) << "Finishing runSetQueryUInt32 init";
...
class NonshardedServerInitializer : public ServerInitializer {
 public:
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
      privacy_sandbox::server_common::log::PSLogContext& log_context,
      microsoft::ANNIndex& ann_index)
      : cache_(cache),
        log_context_(log_context),
        microsoft_ann_index_(ann_index) {
  }
#else
      privacy_sandbox::server_common::log::PSLogContext& log_context)
  ...
  absl::StatusOr<ShardManagerState> InitializeUdfHooks(
      ...
      RunSetQueryStringHook& run_query_hook,
      RunSetQueryUInt32Hook& run_set_query_uint32_hook,
      RunSetQueryUInt64Hook& run_set_query_uint64_hook) override {
    ShardManagerState shard_manager_state;
    auto lookup_supplier = [&cache = cache_]() {
      return CreateLocalLookup(cache);
    };
#if defined(MICROSOFT_AD_SELECTION_BUILD)
    auto ann_lookup_supplier = [&ann_index = microsoft_ann_index_]() {
      return microsoft::AnnLookup::CreateAnnLookup(ann_index);
    };
    InitializeUdfHooksInternal(
        std::move(lookup_supplier), std::move(ann_lookup_supplier),
        get_nearest_neighbors_hook, string_get_values_hook,
        binary_get_values_hook, run_query_hook, run_set_query_uint32_hook,
        run_set_query_uint64_hook, log_context_);
#else
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  microsoft::ANNIndex& microsoft_ann_index_;
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
};
 ...
 public:
  ...
  absl::StatusOr<ShardManagerState> InitializeUdfHooks(
    ...
    InitializeUdfHooksInternal(
        ...
        run_set_query_uint32_hook, run_set_query_uint64_hook, log_context_);
#else
    InitializeUdfHooksInternal(
