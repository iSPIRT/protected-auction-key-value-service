...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
TEST_F(UdfClientTest,
       MICROSOFT_JsMicrosoftRegisterGetNearestNeighborsHookCallSucceeds) {
  auto mock_lookup = std::make_unique<MockLookup>();
  ...
  TextFormat::ParseFromString(
      R"pb(kv_pairs {
             ...
             value {
               keyset_values {
                 values: "embedding_number1"
                 values: "embedding_number2"
                 values: "embedding_number3"
               }
             }
           })pb",
      &response);
  ON_CALL(*mock_lookup, GetKeyValueSet(_, _)).WillByDefault(Return(response));

  auto get_nns_hook = GetNearestNeighborsHook::Create();
  get_nns_hook->FinishInit(std::move(mock_lookup));
  UdfConfigBuilder config_builder;
  absl::StatusOr<std::unique_ptr<UdfClient>> udf_client =
      UdfClient::Create(std::move(
          config_builder.MicrosoftRegisterGetNearestNeighborsHook(*get_nns_hook)
              .SetNumberOfWorkers(1)
              .Config()));
  ...
  absl::Status code_obj_status = udf_client.value()->SetCodeObject(CodeConfig{
      .js = R"(
          function hello(input) {
            let kvPairs = JSON.parse(getNearestNeighbors([input])).kvPairs;
            let output = "";
            for(const key in kvPairs) {
              output += "Key: " + key;
              if (kvPairs[key].hasOwnProperty("keysetValues")) {
                output += ", Values: " + kvPairs[key].keysetValues.values;
              }
            }
            return output;
          }  )",
  ...
  EXPECT_EQ(
      *result,
      R"("Key: key1, Values: embedding_number1,embedding_number2,embedding_number3")");
