...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
constexpr char kMicrosoftStringGetNearestNeighborsHookJsName[] =
    "getNearestNeighbors";
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
UdfConfigBuilder& UdfConfigBuilder::MicrosoftRegisterGetNearestNeighborsHook(
    GetNearestNeighborsHook& get_nearest_neighbors_hook) {
  auto get_nns_function_object = std::make_unique<
      FunctionBindingObjectV2<std::weak_ptr<RequestContext>>>();
  get_nns_function_object->function_name =
      kMicrosoftStringGetNearestNeighborsHookJsName;
  get_nns_function_object->function =
      [&get_nearest_neighbors_hook](
          FunctionBindingPayload<std::weak_ptr<RequestContext>>& in) {
        get_nearest_neighbors_hook(in);
      };
  config_.RegisterFunctionBinding(std::move(get_nns_function_object));
  return *this;
}
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
