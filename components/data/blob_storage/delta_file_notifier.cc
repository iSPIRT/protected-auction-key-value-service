...
absl::StatusOr<bool> DeltaFileNotifierImpl::ShouldListBlobs(
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  auto* target_map = &prefix_start_after_map;
  if (MicrosoftIsAnnSnapshotFilename(notification_blob.key)) {
    target_map = &prefix_start_after_map_ann;
  }
  if (auto iter = target_map->find(notification_blob.prefix);
      iter != target_map->end() && notification_blob.key < iter->second) {
  ...
  return blob_prefix_allowlist_.Contains(notification_blob.prefix) &&
         ...
         (IsDeltaFilename(notification_blob.key) ||
          MicrosoftIsAnnSnapshotFilename(notification_blob.key));
#else
         IsDeltaFilename(notification_blob.key);
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
}
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  absl::flat_hash_map<std::string, std::string> prefix_start_after_map_ann;
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
    auto prefix_blobs_map_ann = ListPrefixDeltaFiles(
        location, blob_prefix_allowlist_, prefix_start_after_map_ann, client_,
        log_context_, FilePrefix<FileType::ANNSNAPSHOT>());
    for (const auto& [prefix, prefix_blobs] : prefix_blobs_map_ann) {
      for (const auto& blob : prefix_blobs) {
        if (!MicrosoftIsAnnSnapshotFilename(blob)) {
          continue;
        }
        callback(prefix.empty() ? blob : absl::StrCat(prefix, "/", blob));
        prefix_start_after_map_ann[prefix] = blob;
        delta_file_count++;
      }
    }
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
