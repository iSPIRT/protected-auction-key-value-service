...
TEST(FileGroupSearchUtilsTest,
     MICROSOFT_ValidateFindingFileGroupsBlobClientEmptyAnnResponse) {
  FileGroupFilter filter{.file_type = FileType::ANNSNAPSHOT};
  BlobStorageClient::DataLocation location{.bucket = "bucket"};
  MockBlobStorageClient blob_client;
  EXPECT_CALL(
      blob_client,
      ListBlobs(Field(&BlobStorageClient::DataLocation::bucket, "bucket"),
                Field(&BlobStorageClient::ListOptions::prefix, "ANNSNAPSHOT")))
      .Times(2)
      .WillRepeatedly(Return(std::vector<std::string>{}));
  auto file_group = FindMostRecentFileGroup(location, filter, blob_client);
  EXPECT_TRUE(file_group.ok()) << file_group.status();
...
TEST(FileGroupSearchUtilsTest, MICROSOFT_ValidateFindingAnnSnapshotFileGroups) {
  MockBlobStorageClient blob_client;
  EXPECT_CALL(
      blob_client,
      ListBlobs(Field(&BlobStorageClient::DataLocation::bucket, "bucket"),
                Field(&BlobStorageClient::ListOptions::prefix, "ANNSNAPSHOT")))
      .WillOnce(Return(std::vector<std::string>{
          "ANNSNAPSHOT_1705430864435450_00000_OF_000003",
          "ANNSNAPSHOT_1705430864435450_00001_OF_000003",
          "ANNSNAPSHOT_1705430864435450_00002_OF_000003",
          "ANNSNAPSHOT_1705430864435451_00000_OF_000002"}));
  auto file_groups = FindFileGroups(
      BlobStorageClient::DataLocation{.bucket = "bucket"},
      FileGroupFilter{.file_type = FileType::ANNSNAPSHOT}, blob_client);
  EXPECT_TRUE(file_groups.ok()) << file_groups.status();
  ASSERT_EQ(file_groups->size(), 2);
  // Verify first file group
  EXPECT_EQ((*file_groups)[0].Size(), 3);
  EXPECT_EQ((*file_groups)[0].Basename(), "ANNSNAPSHOT_1705430864435450");
  EXPECT_EQ((*file_groups)[0].GetStatus(), FileGroup::FileStatus::kComplete);
  EXPECT_EQ((*file_groups)[0].Type(), FileType::ANNSNAPSHOT);
  EXPECT_THAT(
      (*file_groups)[0].Filenames(),
      UnorderedElementsAre("ANNSNAPSHOT_1705430864435450_00000_OF_000003",
                           "ANNSNAPSHOT_1705430864435450_00001_OF_000003",
                           "ANNSNAPSHOT_1705430864435450_00002_OF_000003"));
  // Verify second file group
  EXPECT_EQ((*file_groups)[1].Size(), 2);
  EXPECT_EQ((*file_groups)[1].Basename(), "ANNSNAPSHOT_1705430864435451");
  EXPECT_EQ((*file_groups)[1].GetStatus(), FileGroup::FileStatus::kPending);
  EXPECT_EQ((*file_groups)[1].Type(), FileType::ANNSNAPSHOT);
  EXPECT_THAT(
      (*file_groups)[1].Filenames(),
      UnorderedElementsAre("ANNSNAPSHOT_1705430864435451_00000_OF_000002"));
}

TEST(FileGroupSearchUtilsTest,
     MICROSOFT_ValidateFindingMostRecentAnnSnapshotFileGroups) {
  MockBlobStorageClient blob_client;
  EXPECT_CALL(
      blob_client,
      ListBlobs(
          Field(&BlobStorageClient::DataLocation::bucket, "bucket"),
          AllOf(Field(&BlobStorageClient::ListOptions::prefix, "ANNSNAPSHOT"),
                Field(&BlobStorageClient::ListOptions::start_after,
                      "start_after"))))
      .Times(2)
      .WillRepeatedly(Return(std::vector<std::string>{
  ...
  ASSERT_TRUE(file_group.ok()) << file_group.status();
  EXPECT_EQ((*file_group)->Size(), 3);
  EXPECT_EQ((*file_group)->Basename(), "ANNSNAPSHOT_1705430864435450");
  EXPECT_EQ((*file_group)->GetStatus(), FileGroup::FileStatus::kComplete);
  EXPECT_EQ((*file_group)->Type(), FileType::ANNSNAPSHOT);
  EXPECT_THAT(
      (*file_group)->Filenames(),
      UnorderedElementsAre("ANNSNAPSHOT_1705430864435450_00000_OF_000003",
                           "ANNSNAPSHOT_1705430864435450_00001_OF_000003",
  ...
  ASSERT_TRUE(file_group.ok()) << file_group.status();
  EXPECT_EQ((*file_group)->Size(), 2);
  EXPECT_EQ((*file_group)->Basename(), "ANNSNAPSHOT_1705430864435451");
  EXPECT_EQ((*file_group)->GetStatus(), FileGroup::FileStatus::kPending);
  EXPECT_EQ((*file_group)->Type(), FileType::ANNSNAPSHOT);
  EXPECT_THAT(
      (*file_group)->Filenames(),
