...
TEST(FileGroupTest, MICROSOFT_ValidateAddingSingleAnnSnapshotFile) {
  auto annsnapshot_file = "ANNSNAPSHOT_1705430864435450";
  FileGroup file_group;
  auto status = file_group.AddFile(annsnapshot_file);
  EXPECT_TRUE(status.ok()) << status;
  EXPECT_EQ(file_group.Basename(), annsnapshot_file);
  EXPECT_EQ(file_group.Size(), 1);
  EXPECT_EQ(file_group.Type(), FileType::ANNSNAPSHOT);
  EXPECT_EQ(file_group.GetStatus(), FileGroup::FileStatus::kComplete);
  EXPECT_THAT(file_group.Filenames(),
              testing::UnorderedElementsAre(annsnapshot_file));
}
