...
def data_targets(data_path, config_path = "", certificate_components = {}):
    ...
    component_data_files = [
        (
            ...
            # microsoft code starts here
            # ann indexes
            [
                struct(
                    name = "ann_snapshot_{}".format(idx),
                    infile = ":{}".format(delta_file),
                    # basename excluding extension
                    outfile = delta_file.rsplit("/", 1)[1].rsplit(".")[0],
                )
                for idx, delta_file in enumerate(native.glob(["{}/ANNSNAPSHOT_*".format(component_data_dir)]))
            ],
            # microsoft code ends here
        )
    ...
    [
        ...
        for component_name, csv_files, _, _ in component_data_files
    ...
    [
        ...
        for component_name, _, js_files, _ in component_data_files
    ...
    [
        copy_file(
            name = "{}-{}".format(component_name, file.name),
            src = file.infile,
            out = _filepath_for(component_name, file.outfile),
        )
        for component_name, _, _, ann_files in component_data_files
    ...
    [
        pkg_files(
            ...
            srcs = [":{}".format(_filepath_for(component_name, f.outfile)) for f in csv_files + js_files + ann_files],
            attributes = pkg_attributes(mode = "0555"),
            prefix = "/{}/data".format(component_name),
        )
        for component_name, csv_files, js_files, ann_files in component_data_files
    ]
    ...
    pkg_zip(
        name = "sut_data",
        srcs = [":data_{}".format(component_name) for component_name, _, _, _ in component_data_files] +
