...
TEST(SnapshotFilename, IsLogicalShardingConfigFilename) {
  ...
  EXPECT_FALSE(IsLogicalShardingConfigFilename("ANNSNAPSHOT_1234512345123451"));
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  EXPECT_FALSE(IsFileGroupFileName("ANNSNAPSHOT"));
  EXPECT_FALSE(IsFileGroupFileName("ANNSNAPSHOT_1234512345123451"));
  EXPECT_FALSE(IsFileGroupFileName("ANNSNAPSHOT_1234512345123451_00000"));
  EXPECT_TRUE(
      IsFileGroupFileName("ANNSNAPSHOT_1234512345123451_00000_OF_000100"));
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
}
...
#if defined(MICROSOFT_AD_SELECTION_BUILD)
  filename =
      ToFileGroupFileName(FileType::ANNSNAPSHOT, /*logical_commit_time=*/10,
                          /*file_index=*/0, /*file_group_size=*/10);
  ASSERT_TRUE(filename.ok()) << filename.status();
  EXPECT_THAT(*filename, "ANNSNAPSHOT_0000000000000010_00000_OF_000010");
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
}

#if defined(MICROSOFT_AD_SELECTION_BUILD)
TEST(SnapshotFilename, MICROSOFT_MicrosoftIsAnnSnapshotFilename) {
  EXPECT_FALSE(MicrosoftIsAnnSnapshotFilename(""));
  EXPECT_FALSE(MicrosoftIsAnnSnapshotFilename("ANNSNAPSHOT_"));
  EXPECT_FALSE(MicrosoftIsAnnSnapshotFilename("ANNSNAPSHOT"));
  EXPECT_FALSE(MicrosoftIsAnnSnapshotFilename("ANNSNAPSHOT_12345123451234510"));
  EXPECT_FALSE(MicrosoftIsAnnSnapshotFilename("ANNSnapshot_1234512345123451"));
  EXPECT_TRUE(MicrosoftIsAnnSnapshotFilename("ANNSNAPSHOT_1234512345123451"));
}
#endif  // defined(MICROSOFT_AD_SELECTION_BUILD)
